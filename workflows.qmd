...
Sun Grid Engine dependencies
```{.sh filename="step1.sh"}
#$ -cwd
command1 --in $1 --out $1.$JOB_NAME
```
```{.sh filename="step2.sh"}
#$ -cwd
command2 --in $1 --out $1.$JOB_NAME
```

```{.sh code-line-numbers="false"}
$ qsub -N step1 step1.sh input1
$ qsub -N step2 -hold_jid step1 step2.sh input1.step1
```
